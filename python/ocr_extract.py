...
def extract_person_names(text):
    ...
    # ── UR Certificate anchor patterns (highest priority) ────────────────────
    ur_patterns = [
        r'CONFERS\s+UPON\s+YOU[\s\n]+([A-Z][A-Z\s\']{4,40}?)[\s\n]+(?:THE\s+DEGREE|IS\s+AWARDED)',
        r'(?:COLLEGE\s+OF\s+[\w\s]+?)[\s\n]+([A-Z][A-Z\s\']{4,40}?)[\s\n]+IS\s+AWARDED',
        r'IS\s+AWARDED\s+THE\s+DEGREE\s+OF[\s\n]+([A-Z][A-Z\s\']{4,40}?)[\s\n]+BACHELOR',
        r'(?:CERTIFY\s+THAT|CERTIFIES\s+THAT)[\s\n]+([A-Z][A-Z\s\']{4,40}?)[\s\n]+(?:HAVING|HAS)',
        r'([A-Z][A-Z\s\']{4,40}?)[\s\n]+IS\s+AWARDED\s+THE\s+DEGREE',
    ]
    for pat in ur_patterns:
        try:
            m = re.search(pat, text_upper, re.MULTILINE | re.DOTALL)
            if m:
                raw = m.group(1)
                cleaned = ' '.join(
                    w for w in re.sub(r"[^A-Z\s']+", ' ', raw).split()
...
def preprocess_for_ocr(img_cv):
    """Apply multiple preprocessing strategies including rotation, return best text."""
    h, w = img_cv.shape[:2]
    results = []
